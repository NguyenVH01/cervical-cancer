...
class PatchMerging2D(nn.Module):
    ...
    def forward(self, x):
        ...
        SHAPE_FIX = [-1, -1]
        if (W % 2 != 0) or (H % 2 != 0):
            print(f"Warning, x.shape {
                  x.shape} is not match even ===========", flush=True)
            SHAPE_FIX[0] = H // 2
            SHAPE_FIX[1] = W // 2
...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.x_proj_weight = nn.Parameter(torch.stack(
            [t.weight for t in self.x_proj], dim=0))  # (K=4, N, inner)
        del self.x_proj
        ...
        self.dt_projs_weight = nn.Parameter(torch.stack(
            [t.weight for t in self.dt_projs], dim=0))  # (K=4, inner, rank)
        self.dt_projs_bias = nn.Parameter(torch.stack(
            [t.bias for t in self.dt_projs], dim=0))  # (K=4, inner)
        del self.dt_projs

        self.A_logs = self.A_log_init(
            self.d_state, self.d_inner, copies=4, merge=True)  # (K=4, D, N)
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        As = -torch.exp(self.A_logs.float()).view(-1,
                                                  # (k * d, d_state)
                                                  self.d_state)
        ...
        **kwargs,
        ...
        self.conv33conv33conv11 = nn.Sequential(
            ...
            nn.Conv2d(in_channels=hidden_dim // 2,
                      out_channels=hidden_dim // 2, kernel_size=1, stride=1),
            nn.ReLU()
        )
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate,
                                                ...
                                                sum(depths))]
        dpr_decoder = [x.item() for x in torch.linspace(
        ...
        for i_layer in range(self.num_layers):
            layer = VSSLayer(
                ...
                d_state=math.ceil(
                    dims[0] / 6) if d_state is None else d_state,  # 20240109
                drop=drop_rate,
                attn_drop=attn_drop_rate,
                drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],
                norm_layer=norm_layer,
                downsample=PatchMerging2D if (
...
medmamba_t = VSSM(depths=[2, 2, 4, 2], dims=[
                  96, 192, 384, 768], num_classes=6).to("cuda")
medmamba_s = VSSM(depths=[2, 2, 8, 2], dims=[
                  96, 192, 384, 768], num_classes=6).to("cuda")
medmamba_b = VSSM(depths=[2, 2, 12, 2], dims=[
                  128, 256, 512, 1024], num_classes=6).to("cuda")
