...
def build_custom_dataset(config, split=[0.7, 0.2, 0.1]):
    ...
    dataset = CustomDataset(config.DATA.DATA_PATH,   transformations=transform_val)
   
    total_len = len(dataset)
    print("=========")
    print(f'Len of custom dataset: {total_len}')
    print("=========")
    ...
    train_dataset, val_dataset, test_dataset = random_split(dataset=dataset, lengths=[tr_len, vl_len, ts_len])
    ...
    return train_dataset, val_dataset, test_dataset, data_loader_train, data_loader_val, data_loader_test, mixup_fn
