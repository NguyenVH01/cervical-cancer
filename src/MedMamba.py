...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        x_dbl = torch.einsum(
            "b k d l, k c d -> b k c l", xs.view(B,
                                                 K, -1, L), self.x_proj_weight
        )
        ...
        dts = torch.einsum(
            "b k r l, k d r -> b k d l", dts.view(B,
                                                  K, -1, L), self.dt_projs_weight
        )
...
class ConvSSM(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        x = input_right + \
            self.drop_path(self.self_attention(self.ln_1(input_right)))
